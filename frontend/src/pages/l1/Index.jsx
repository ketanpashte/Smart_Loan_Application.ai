...
import { EyeIcon, MagnifyingGlassIcon, FunnelIcon, ClockIcon } from '@heroicons/react/24/outline';
...
const L1Index = () => {
  ...
  useEffect(() => {
    const fetchApplications = async () => {
      setLoading(true);
      try {
        const response = await api.get('/api/workflow/l1/applications');
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case 'PENDING_L1':
        return <span className="status-pending">Pending L1 Review</span>;
      case 'L1_APPROVED':
        return <span className="status-approved">L1 Approved</span>;
      case 'L1_REJECTED':
        return <span className="status-rejected">L1 Rejected</span>;
      default:
    ...
    <Layout title="L1 Manager Review">
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">L1 Manager Review</h1>
            <p className="text-gray-600 mt-1">Review RCPU completed applications</p>
          </div>
          <div className="mt-4 sm:mt-0 flex space-x-4">
            <span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 text-blue-800">
              {filteredApplications.filter(app => app.status === 'PENDING_L1').length} Pending
            </span>
            <span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-green-100 text-green-800">
              {filteredApplications.filter(app => app.status === 'L1_APPROVED').length} Approved
            </span>
          </div>
        </div>
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div>
                        ...
                        <div className="text-sm text-gray-500">{formatDate(application.rcpuCompletedDate)}</div>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center text-sm text-gray-900">
                        <ClockIcon className="h-4 w-4 mr-1 text-gray-400" />
                        {calculateDaysInQueue(application.rcpuCompletedDate)} days
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
